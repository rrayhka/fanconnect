...
$news   = getNewsById($_GET['news']);


if($_SERVER['REQUEST_METHOD'] == 'POST')
{
    editNews($_POST, $_SESSION["email"], $_GET['news'], getNewsCover($_GET['news'], $_FILES['news_cover'], $news['news_cover']));
    unset($_POST);
...
<section class="user-favourites">
    ...
    <div class="card-base-info card-favourites-content">
        <div class="card-base-info-title">
            ...
            <h1 class="font-kanit">Edit Content</h1>
        </div>
        <div class="group-information-news">
            <form action="" method="post" enctype="multipart/form-data">
            <div class="auth-body">
                <div class="profile-edit-input news-header">
            ...
            background-size:cover;background-position:center" id="news-cover"></div>
                    <div class="form-input">
                        <input type="file" class="font-montserrat" name="news_cover" id="file-cover" onchange="coverPreview()">
                    </div>
                    <div class="form-input">
                        <span class="font-kanit">News Title</span>
                        <input type="text" class="font-montserrat" name="news_title" placeholder="News Title" value="<?= $news["news_title"] ?>" required>
                    </div>
                    <div class="form-input">
                        <span class="font-kanit">Group</span>
                        <select name="id_group">
                            <?php foreach($groups as $group) : ?>
                            <option value="<?= $group["id_group"] ?>" <?php if($news["id_group"] == $group["id_group"]) echo "selected";  ?>><?= $group["group_name"] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <input type="hidden" name="news_content" id="news-content" value='<?= $news["news_content"] ?>'>
                    <button type="submit" class="primary-btn font-kanit">Edit News</button>
